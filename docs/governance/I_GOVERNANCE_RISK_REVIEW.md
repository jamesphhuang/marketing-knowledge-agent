...
| GR-2 | 橡皮圖章 review（decision=建議、reviewer 空）被 apply | high | high（已發生前半） | J 已加：reviewer/reviewed_at 空 → warning、IN-1（全部 decision==建議 且 rows>10）→ info；硬擋仍待 K 前置檢查 | J 的必填 warning + IN-1 | J sprint 測試 | 部分修復（validation 層偵測完成；apply 層硬擋待 K） |

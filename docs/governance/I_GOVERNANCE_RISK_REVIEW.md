...
| GR-9 | pending_metric / can_quote=false 內容在外部用途被引用（無 intent gating） | high | low（離線層已關閉）→ high（接 LLM 後需重新驗證） | P sprint 已完成本地 `SearchFilters.intent` 強制規則、查詢前 denylist pre-check、abstention 與 external citation 出口斷言；Stage 3 接 LLM 時仍需依 H §8 對 LLM 輸出再過 denylist | `SearchFilters.intent` 強制規則 | EV-G1~G6 + query gating tests | 已修（P sprint：離線層 gating 完成；Stage 3 的 LLM 輸出檢查留待驗收） |

...
## 2026-07-11 加分機制不得洩漏進閘門判斷
- 情境:Slack 實測問一個不在索引的品牌,系統卻正常回答並標「可對外引用」。
- 根因:abstention 的相關度判斷用了 rerank 分(含 freshness +0.06 加分),
  零相關結果被加分灌到剛好 =0.1 門檻、擦邊通過。
- 修正:閘門判斷改用加分前的原始檢索 score;rerank 加分只影響排序不影響拒答。
- 通則:任何「加分/排序」機制(rerank bonus)不得洩漏進「該不該擋」的閘門判斷
  ——閘門一律用未加工的原始分數/原始欄位(同 P sprint intent chokepoint 的精神)。
- 只有真實資料(Slack live smoke)才會踩到:合成測試的相關文件分數夠高,壓線案例碰不到。
